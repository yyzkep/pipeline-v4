...
public:
	...
	virtual preview_image_ret_val_t get_preview_image_properties(int* width, int* height, ImageFormat* imageFormat, bool* isTranslucent) const = 0;
	virtual preview_image_ret_val_t get_preview_image(unsigned char* data, int width, int height, ImageFormat imageFormat) const = 0;
	...
	virtual void get_reflectivity(vector_t& reflect) = 0;
	...
	virtual void set_shader_and_params(kvalues* pKeyValues) = 0;
...
inline bool is_error_material(i_material* pMat)
{
	return !pMat || pMat->is_error_material();
}